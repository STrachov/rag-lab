...
- SavedExperiment uses backend-generated `raglab.saved_experiment.v1` snapshots resolved through historical index/chunks/data lineage. Configuration `params_hash` is separate from exact GT/chunks/sparse byte hashes. One atomic `created -> running` claim consumes the evaluation attempt; retries require new experiments. Code provenance is captured at execution. Old development experiments/caches/GT must be recreated; no legacy schema readers. See `docs/REPRODUCIBILITY.md`.
