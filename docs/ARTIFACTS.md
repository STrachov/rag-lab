...
Chunking preview in the Chunking section is a computed API response by default. It may show chunk
...
sparse scores. It also creates or reuses `DerivedCache(cache_type="retrieval_temp")` for the retrieved
candidate set. Reranking reads that retrieval cache plus full materialized chunk text locally from the
chunks cache, then returns optional rerank scores. It does not add full text to Qdrant payloads or
retrieval temp metadata. It is still debug output, not a saved experiment result.
